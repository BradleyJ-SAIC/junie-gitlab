...
2. In the CI/CD settings you need to create the required variables:
   ...
   **Or**, instead of `JUNIE_API_KEY`, you can provide your own API key for one of the supported providers (BYOK — see the table below) or a [LiteLLM proxy](https://junie.jetbrains.com/docs/custom-llm-litellm.html) URL (see the table below).
...
| `JUNIE_LITELLM_URL`            | `null`          | LiteLLM proxy base URL (alternative to `JUNIE_API_KEY`)                     |
| `JUNIE_LITELLM_API_KEY`        | `null`          | LiteLLM proxy API key. Optional — omit it if the proxy requires no auth     |
